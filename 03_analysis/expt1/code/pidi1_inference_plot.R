...
expt1_inference_child_plot <- expt1_inference_child %>%
  filter(!is.na(age_exact)) %>%
  distinct(id, .keep_all = TRUE)

expt1_inference_adult_plot <- expt1_inference_adult %>%
  distinct(id, .keep_all = TRUE)


expt1_inference_summary_adult = summarySE(data = expt1_inference_adult,
                                                 measurevar = "inf",
...
expt1_plot_inference <- ggplot() +
  geom_point(
    data = expt1_inference_child_plot, 
    aes(x = age_exact,
        y = inf_avg, 
        color = condition,
        shape = condition),
  ...
  geom_point(
    data = expt1_inference_adult_plot,
  ...
  geom_errorbar(
    data = expt1_inference_summary_adult,
    aes(ymin = inf - ci,
        ymax = inf + ci,
