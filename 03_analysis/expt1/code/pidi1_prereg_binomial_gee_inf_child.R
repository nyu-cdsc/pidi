...
expt1_first_180_inference_child_gee <- expt1_first_180_inference_child %>%
  filter(!is.na(age_exact))  %>% 
  arrange(age_categorical, id, trial_order)


expt1_first_180_inference_child_gee$age_categorical <- as.factor(
  as.character(expt1_first_180_inference_child_gee$age_categorical))

expt1_first_180_inference_child_results <- geeglm(
  inf ~ condition + age_exact + condition*age_exact, 
  data = expt1_first_180_inference_child_gee, 
  family = binomial(logit), 
  id = new_id, 
  corstr = "exchangeable")
summary(expt1_first_180_inference_child_results)
expt1_first_180_inference_child_anova <- anova(expt1_first_180_inference_child_results)
expt1_first_180_inference_child_anova
...
#   emtrends(expt1_first_180_inference_child_results, 
#            ~ condition, 
#            var = "age_exact"))
# 
