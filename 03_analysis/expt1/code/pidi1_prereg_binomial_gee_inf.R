...
expt1_first_180_inference <- expt1_first_180_inference %>% 
  arrange(age_categorical, id, trial_order)

expt1_first_180_inference$age_categorical <- as.factor(
  as.character(expt1_first_180_inference$age_categorical))

expt1_first_180_inference_results <- geeglm(
  inf ~ condition + age_categorical + condition*age_categorical, 
  data = expt1_first_180_inference, 
  family = binomial(logit), 
  id = new_id, 
  corstr = "exchangeable")
summary(expt1_first_180_inference_results)
expt1_first_180_inference_anova <- anova(expt1_first_180_inference_results)
expt1_first_180_inference_anova
