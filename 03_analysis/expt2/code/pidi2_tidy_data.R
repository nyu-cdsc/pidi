...
expt2 <- expt2 %>%
  ...
  mutate(
    stimulus = 
      ifelse(grepl("3_[1-4]d", expt2$stim1), "same", 
             ifelse(grepl("3_[1-4]c", expt2$stim1), "opposite", NA)
      )
  ) %>%
  # Creating a column for what property participants learned about    
  mutate(
    property = 
      ifelse(grepl("3_1", expt2$stim1), "pizza", 
             ifelse(grepl("3_2", expt2$stim1), "paint",
                    ifelse(grepl("3_3", expt2$stim1), "piano",
                           ifelse(grepl("3_4", expt2$stim1), "tree", NA))))
  ) %>%  
  # Creating a column specifying type of Trial
  mutate(
    trialType = 
      case_when(
        grepl("PiDi_2", expt2$stim1) | expt2$stim1 == "AudioCheck" ~ "attnCheck",
        startsWith(stim1, "PiDi_3") ~ "test",
        grepl("PiDi_[4-5]", expt2$stim1) ~ "pragTest",
